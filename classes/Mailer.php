...
class Mailer {
	...
	static public function sendmail($data=array())
    {
		...
		*/
		...
		try {
			...
			if(!empty($data['host'])){
				...
				if(!isset($data['SMTPSecure'])) $secure=''; else $secure=$data['SMTPSecure'];
            ...
            if(!empty($data['attachments'])){
                foreach($data['attachments'] as $v){
                    if(is_array($v)) static::$mail->addAttachment($v[0],$v[1]); else static::$mail->addAttachment($v);
                }
            }
			...
			static::$errors=$e->errorMessage(); //Pretty error messages from PHPMailer
			//Log_Base::DBLog(....);
			return false;
		} catch (Exception $e) {
