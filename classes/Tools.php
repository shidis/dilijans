...
class Tools
{
    ...
    static private $specSymbols=array(".",",","-","_"); // символы, которые могут быть в урле, но не могут повторяться
    static private $trimSpecSymbols=array(".",",","-","_"); // запрещенные пограничный символы (подмножество specSymbols)
    ...
    */
    ...
    static public function str2iso($s,$len=-1,$reg='lower',$iso_table='')
    {
        ...
        do {
            ...
            foreach(Tools::$specSymbols as $v)
                while($s!=($s0=str_replace($v.$v,$v,$s)))  {$i++;$s=$s0;}

            // удаляем особые комбинации
            $j=0; 
            while($s!=($s0=str_replace("-_",'-',$s))) {$j++;$s=$s0;}
            while($s!=($s0=str_replace("_-",'-',$s))) {$j++;$s=$s0;}
        ...
        do {
            ...
            foreach(Tools::$trimSpecSymbols as $v){
                if(mb_substr($s,0,1)==$v) {$s=Tools::_substr_replace($s,'',0,1);$i++;}
                if(mb_substr($s,mb_strlen($s)-1,1)==$v) {$s=Tools::_substr_replace($s,'',mb_strlen($s)-1,1);$i++;}
            }
        } while($i);

        if($reg=='lower') $s=mb_strtolower($s); elseif($reg=='upper') $s=mb_strtoupper($s);
    ...
    {
        $email=trim($email);
        if(!empty($email) && preg_match('|^([a-z0-9_\.\-]{1,20})@([a-z0-9\.\-]{1,20})\.([a-z]{2,4})$|uis', $email)) return true; else return false;
    }
    ...
    {
        if(preg_match("/([0-9]{2})-([0-9]{2})-([0-9]{4})/",$dt,$m))
    ...
    {
        if(preg_match("/([0-9]{4})-([0-9]{2})-([0-9]{2})/",$dt,$m))
            if($shortYear) return "{$m[3]}{$delimeter}{$m[2]}{$delimeter}".mb_substr($m[1],2,2); else return "{$m[3]}{$delimeter}{$m[2]}{$delimeter}{$m[1]}";

        elseif(preg_match("/([0-9]{2})-([0-9]{2})-([0-9]{4})/",$dt,$m))
            if($shortYear) return "{$m[1]}{$delimeter}{$m[2]}{$delimeter}".mb_substr($m[3],2,2); else return "{$m[1]}{$delimeter}{$m[2]}{$delimeter}{$m[3]}";
    ...
    {
        if(preg_match("/([0-9]{2}):([0-9]{2}):([0-9]{2})/",$dt,$m))
            if($noSec)
                return("{$m[1]}{$delimeter}{$m[2]}");
    ...
    {
        if(preg_match("/([0-9]{4})-([0-9]{2})-([0-9]{2}) ([0-9]{2}):([0-9]{2}):([0-9]{2})/",$dt,$m))
            return("{$m[3]}{$delimeter}{$m[2]}{$delimeter}{$m[1]} {$m[4]}:{$m[5]}:{$m[6]}");

        elseif(preg_match("/([0-9]{2})-([0-9]{2})-([0-9]{4}) ([0-9]{2}):([0-9]{2}):([0-9]{2})/",$dt,$m))
            return("{$m[1]}{$delimeter}{$m[2]}{$delimeter}{$m[3]} {$m[4]}:{$m[5]}:{$m[6]}");
    ...
    static public function dateArr($dt)
    {
        ...
        if (count($m)<3) return(false); else
            return array(1=>$m[3],2=>$m[2],3=>$m[1]);
    }
    ...
    */
    ...
    static public function usortStr($a=array(),$order='ASC')
    {
    ...
    static public function uasortStr($a=array(),$order='ASC')
    {
    ...
    static public function uksortStr($a=array(),$order='ASC')
    {
    ...
    */
    ...
    {
        array_walk_recursive ($inArray,'Tools::iconv_array_callback',array($inCharset, $outCharset));
        return $inArray;
    }	
    ...
    static public function otherS($str,$delimeter=',',$emptyIfNoDelimeter=true)
    {
        ...
        elseif($emptyIfNoDelimeter) return ''; else return $str;
    }
    ...
    static public function mb_strcasecmp($str1,$str2)
    {
        ...
        if(preg_match("/^".preg_quote($str1,'/')."$/iu",$str2)) return 0; else return 1;  //37000
    ...
    */
    ...
    static function paginator($baseUrl, $urlParam, $page, $num, $limit, $pageVar='page', $tpl=array('active'=>'','noActive'=>'','dots'=>''), $itemNum=15)
    {
    ...
    {     
        return str_replace(Array('series', 'Series'), Array('серии', 'Серии'), $string);
    }
        ...
        if(!empty($content)) {
            ...
            if(!empty($matches[0])) {
                ...
                foreach ($matches as $match) {
                    $matchSTR = str_replace(Array("[", "]"), "", $match);
