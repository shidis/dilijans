...
cb.load=function()
{if(!cb.isLoaded()){$.ajax({dataType:'html',url:'/ax/callbackForm',success:function(r){$('body').append(r);$('#callback-popup a.close').click(cb.close);$('#callback-popup form').submit(function(e){e.preventDefault();var canSend=true;if(canSend){$.ajax({url:'/ax/callback',data:{f:$('#callback-popup form').serialize()},success:function(r){if(r.fres){$('#callback-popup form').html('Спасибо, наш менеджер свяжется с вами ближайшее время');if(typeof _gaq!='undefined'){_gaq.push(['_trackEvent','Interface','callBackFormSend','callBackFormSend;']);}}else msg('Сообщение не отправлено. '+r.fres_msg);}});}});cb.open();}});}else cb.open();}
