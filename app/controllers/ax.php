...
class App_Ax_Controller extends App_Common_Controller
{
    ...
    public function geoCity()
    {
        ...
        $this->r['fres'] = true;
        $this->r['geo'] = array('ip' => $ip);
        $geo = new App_Geo(true);
        $res = $geo->resolveWithMapsTable($ip);
        if ($res !== false) $this->r['geo'] = array_merge($this->r['geo'], $res); else {
            $this->r['fres'] = false;
    ...
    public function regionDeliveryCost()
    {
        ...
        if ($this->cityId == -77 || empty($this->cityId)) $this->r['fres'] = "Стоимость доставки по Москве составляет " . Data::get('delivery_cost') . ' руб';
            ...
            if ($gr == 2) {
                $width = @$f['w'];
                $radius = @$f['d'];
                $dcost = App_Dostavka::getCost(2, array('d' => $radius, 'j' => $width, 'city_id' => $this->cityId, 'am' => 4));
                if ($dcost !== false) {
                ...
                $height = @$f['h'];
                $radius = @$f['d'];
                $dcost = App_Dostavka::getCost(1, array('d' => $radius, 'w' => $width, 'h' => $height, 'city_id' => $this->cityId, 'am' => 4));
                if ($dcost !== false) {
    ...
    public function galleryForm()
    {
        ...
        if (@$_REQUEST['gr'] == 1) $this->h1 = 'Фото шин в эксплуатации'; else $this->h1 = 'Фото дисков в эксплуатации';
    }
    ...
    public function callback()
    {
        ...
        if (!Mailer::sendmail(array('fromAddr' => $fromAddr, 'fromName' => $fromName, 'toAddr' => $toAddr, 'toName' => $name, 'body' => $body, 'subject' => $subj))) {
            $this->r['fres'] = false;
    ...
    public function delveryCostByCity()
    {
        ...
        if ($gr == 2) {
            $j = (float)@$_REQUEST['j'];
            $this->r['d'] = Dostavka::getCost($gr, array('j' => $j, 'd' => $d, 'city_id' => $id, 'am' => 4));
        } else {
            $w = (float)@$_REQUEST['w'];
            $h = (float)@$_REQUEST['h'];
            $this->r['d'] = Dostavka::getCost($gr, array('w' => $w, 'h' => $h, 'd' => $d, 'city_id' => $id, 'am' => 4));
        }
    }
    ...
    public function scl()
    {
        ...
        if(!empty($ids))
            if(!empty($this->os->adminCfg['reservation']['futureSuplr']) && !empty($this->os->adminCfg['reservation']['futureSuplr']['deliveringStateId']) && !empty($this->os->adminCfg['delivery']['DBF_deliveryDate'])) {
