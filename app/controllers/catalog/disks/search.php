...
class App_Catalog_Disks_Search_Controller extends App_Catalog_Disks_Common_Controller
{
    public
        // главные параметры
    ...
    public
        $apMode = 0;  // _GET['ap']==1  включает delta_et  и delta_dia
...
//        if(count($this->P2)==1 && count($this->P3)==1 && (count($this->P4)==1  && count($this->P6)==1 || count($this->P46)==1) && count($this->P5)==1){
    ...
    private function _cat()
    {
        ...
        if (!empty($this->P3))
            if ($this->apMode) {
                ...
                $r['P3'] = array('from' => (float)min($this->P3) + $this->_deltaDiaMin, 'to' => (float)min($this->P3) + $this->_deltaDiaMax);
            } else $r['P3'] = array('list' => $this->P3);

        if (!empty($this->P1))
            if ($this->apMode) {
                //$r['P1'] = array('from' => (float)min($this->P1) + $this->_deltaET, 'to' => (float)max($this->P1) + $this->deltaET_);
				$r['P1'] = array('list' => $this->P1);
        ...
        switch ($this->sortBy) {
            default:
                $r['order'][] = 'cc_brand.replica DESC, cc_brand.pos DESC, m_pos ASC, cc_cat.P5';
                break;
            case 1:
        ...
        if (!empty($this->P3_))
            if ($this->apMode) {
                ...
                $r['P3'] = array('from' => (float)min($this->P3_) + $this->_deltaDiaMin, 'to' => (float)min($this->P3_) + $this->_deltaDiaMax);
    ...
    private function _catDouble()
    {
        ...
        if (!empty($this->P2)) $r['P2'] = array('list' => $this->P2);
        if (!empty($this->P4)) $r['P4'] = array('list' => $this->P4);
        if (!empty($this->P5)) $r['P5'] = array('list' => $this->P5);
        if (!empty($this->P6)) $r['P6'] = array('list' => $this->P6);
        ...
        if (!empty($this->sP4)) $r['P4'] = array('list' => $this->sP4);
        if (!empty($this->sP5)) $r['P5'] = array('list' => $this->sP5);
        if (!empty($this->sP6)) $r['P6'] = array('list' => $this->sP6);

        if (!empty($this->sP3))
            if ($this->apMode) {
                //$r['P3'] = array('from' => (float)min($this->sP3) + $this->_deltaDiaMin, 'to' => '');
                $r['P3'] = array('from' => (float)min($this->sP3) + $this->_deltaDiaMin, 'to' => (float)min($this->sP3) + $this->_deltaDiaMax);
            } else $r['P3'] = array('list' => $this->sP3);

        if (!empty($this->sP1))
            if ($this->apMode) {
                //$r['P1'] = array('from' => (float)min($this->sP1) + $this->_deltaET, 'to' => (float)max($this->sP1) + $this->deltaET_);
				$r['P1'] = array('list' => $this->sP1);
            } else $r['P1'] = array('list' => $this->sP1);
        ...
        */
        ...
        $this->ex = array('brand' => array(0 => array(), 'replica' => array()));
                                ...
                                if ($v1['P6'] == $v2['P6'])
                                    if (Tools::mb_strcasecmp(Tools::unesc($v1['csuffix']), Tools::unesc($v2['csuffix'])) === 0) {
                                        ...
                                        if ($v1['replica'])
                                            ...
                                            $this->ex['brand']['replica'][$v1['brand_id']] = array('name' => Tools::unesc($v1['bname']), 'sname' => Tools::unesc($v1['brand_sname']));

                                        // проверка по уточняющим фильтрам
                                        if (empty($this->_brands) || in_array($v1['brand_id'], $this->_brands)) {
        ...
        if (count(@$this->ex['brand']['replica']) > 1) {
            $this->lfi++;
            if (!isset($this->lf['_rbids'])) $this->lf['_rbids'] = array();
            $si = '/' . App_Route::_getUrl('dCat');
            foreach ($this->ex['brand']['replica'] as $k => $v) {
        ...
        // добавляем гет параметры в форму как hidden
        foreach (Url::$sq as $k => $v)
            if (in_array($k, array('p1', 'p2', 'p3', 'p4', 'p5', 'p6', 'sv', 'ap', 'replica', 'vendor', 'q', 'p1_', 'p2_', 'p3_', 'p4_', 'p5_', 'p6_')))
                if (is_array($v)) {
                    foreach ($v as $k1 => $v1) $this->lfh["{$k}[{$k1}]"] = $v1;
                } else  $this->lfh[$k] = $v;
    ...
    public function search()
    {
        ...
        $this->view('catalog/disks/search');
        ...
        if ($this->replica) {
            ...
            if (!empty($this->ab->fname) && !empty(Url::$sq['ap']) && false) { // выключили
        ...
        $this->referer = (strstr(@$_SERVER['HTTP_REFERER'], @Namespaces::$names['app'][server_loc]['aliases'][0][0]) || strstr(@$_SERVER['HTTP_REFERER'], @Namespaces::$names['app'][server_loc]['aliases'][0][1])) ? "<a class='back_button' href='{$_SERVER['HTTP_REFERER']}'>Вернуться назад</a>" : '';
    ...
    {
        $this->searchTpl = 'catalog/disks/searchDouble';
        ...
        if (count($this->P2) == 1 && count($this->P5) == 1) {
            $this->size .= ' ' . current($this->P2) . 'x' . current($this->P5);
            $this->size_ .= ' ' . current($this->sP2) . 'x' . current($this->sP5);
        } elseif (count($this->P5) == 1) {
            $this->size .= ' R' . current($this->P5);
            $this->size_ .= ' R' . current($this->sP5);
        }
        ...
        if (count($this->P46) == 1) {
            ...
            if (count($this->P4) == 1 && count($this->P6) == 1) $this->size .= ' ' . current($this->P4) . 'x' . current($this->P6);
            if (count($this->sP4) == 1 && count($this->sP6) == 1) $this->size_ .= ' ' . current($this->sP4) . 'x' . current($this->sP6);
        }

        if (!empty($this->P1)) {
            $this->size .= ' ET ' . implode(', ', $this->P1);
            $this->size_ .= ' ET ' . implode(', ', $this->sP1);
        }

        if (!empty($this->P3)) {
            $this->size .= ' DIA ' . implode(', ', $this->P3);
            $this->size_ .= ' DIA ' . implode(', ', $this->sP3);
        }

        $this->size = trim($this->size);
        $this->size_ = trim($this->size_);

        $this->breadcrumbs['диски'] = array('/' . App_Route::_getUrl('dCat') . '.html', 'купить литые диски');

        if ($this->replica) {
            $this->title = Tools::cutDoubleSpaces("Диски реплика {$this->bname} {$this->size} - {$this->size_}");
            $this->_title = Tools::cutDoubleSpaces("Результаты поиска: диски Replica {$this->bname} {$this->size} в спарке с {$this->size_}");
            $this->keywords = Tools::cutDoubleSpaces("колесные диски реплика replica {$this->bname} {$this->size} {$this->size_}");
            $this->description = Tools::cutDoubleSpaces("колесные литые диски реплика (replica) {$this->bname} {$this->size} в спарке с {$this->size_}, здесь вы можете выбрать и купить спаренные размеры дисков реплика {$this->balt1} {$this->size} и {$this->size}");

            $this->breadcrumbs['Replica'] = array('/' . App_Route::_getUrl('replicaCat') . '.html', 'купить литые диски');
            ...
            if (!empty($this->ab->fname) && !empty(Url::$sq['ap'])) {
                $this->title = Tools::cutDoubleSpaces("Диски {$this->bname} {$this->size} - {$this->size_}  для {$this->ab->fname}");
                $this->_title = Tools::cutDoubleSpaces("Диски {$this->bname} {$this->size} в спарке с {$this->size_}  для {$this->ab->fname}");
            } else {
                $this->title = Tools::cutDoubleSpaces("Диски {$this->bname} {$this->size} - {$this->size_}");
                $this->_title = Tools::cutDoubleSpaces("Результаты поиска: диски {$this->bname} {$this->size} в спарке с {$this->size_}");
            }

            $this->keywords = Tools::cutDoubleSpaces("литые колесные диски {$this->bname} {$this->size} {$this->size_}");
            $this->description = Tools::cutDoubleSpaces("литые диски {$this->bname} {$this->size} в спарке с размером {$this->size_}, здесь вы можете выбрать и купить спаренные размеры дисков {$this->balt1} {$this->size} с {$this->size_}");

            if ($this->brand_id)
                $this->breadcrumbs[$this->bname] = array('/' . App_Route::_getUrl('dCat') . '/' . $this->brand_sname . '.html', 'литые диски ' . $this->balt);
        }

        $this->breadcrumbs["{$this->balt1} {$this->size} - {$this->size_}"] = '';

        if (empty($this->cat)) {
            $this->qtext = "К сожалению, мы не можем сейчас вам предложить спаренные размеры  дисков  {$this->bname} {$this->size} и {$this->size_}.";
        }
    ...
    public function axView()
    {
        ...
        if (empty($this->cat)) $this->qtext = "Дисков в данный момент нет в наличии";
        // Вывод
        global $app;
