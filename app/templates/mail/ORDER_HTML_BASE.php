<!-- Отправляется при оформлении заказа менеджеру и на коллективный адрес офиса. Тип плательщика: физик-->
<?
$ss=new Content();
?>
<html>
<head>
    <title>Заказ № <?=$orderNum?> <?=!empty($quickOrderNotif)?'*1 click* ':(!empty($productIncomingNotif)?'*сообщить о поступлении*':'')?> на сайте <?=$siteName?></title>
    <style type="text/css">
        BODY,TD,TH{
...
<? if(!empty($quickOrderNotif)){?>
    <p><b>Оформлен быстрый заказ.</b> Необходимо согласование товарных позиций и контактных данных.</p>
<? }?>
