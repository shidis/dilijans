<div class="box-padding">
    ...
    if(!empty($lfi) && App_Route::_getAction()!='search'){
        ...
        <div class="box-grey-01 ext_filter">
            <form action="/<?=App_Route::_getUrl('dSearch')?>.html" class="form-style-01<?=!@$sMode?' livef':''?>">
                ...
                <table class="model_search_filter"><?
                    ...
                    if(!empty($lf['_bids'])){
                        ...
                        ?><tr<?=$i==$lfi?' class="last"':''?> group="_bids">
                            <td class="black" width="120px"><b>Производители:</b></td>
                            <td class="black"><?
                                foreach($lf['_bids'] as $k=>$v){
                                    ?><label<?=$v['chk']?' class="active"':''?>><input type="checkbox"<?=$v['chk']?' checked':''?> name="_bids[<?=$k?>]" id="<?=$v['id']?>"  value="1"><i></i><span class="sp-label-wrapper"><button type="submit" class="mobile-result-label">Найти <span class="mobile-result-label__value"> <b>-</b> шт.</span></button><?=$v['anc']?></span></label><?
                                }
                            ?></td><?
                        ?></tr><?
                    }
                    ...
                    if(!empty($lf['_rbids'])){
                        $i++
                        ?><tr<?=$i==$lfi?' class="last"':''?>>
                            <td class="black" width="120px"><b>Реплика:</b></td>
                        ...
                        ?><tr<?=$i==$lfi?' class="last"':''?> group="_p5">
                            ...
                            <td class="black"><?
                                foreach($lf['_p5'] as $k=>$v){
                                    ?><label<?=$v['chk']?' class="active"':''?>><input type="checkbox"<?=$v['chk']?' checked':''?> name="_p5[<?=$k?>]" id="<?=$v['id']?>"   value="1"><i></i><span class="sp-label-wrapper"><button type="submit" class="mobile-result-label">Найти <span class="mobile-result-label__value"> <b>-</b> шт.</span></button><?=$v['anc']?></span></label><?
                                }
                            ?></td><?
                        ?></tr><?
                    }
                    ...
                    if(!empty($lf['_sv'])){
                        ...
                        ?><tr<?=$i==$lfi?' class="last"':''?> group="_sv">
                            <td class="black" width="120px"><b>Сверловки:</b></td>
                            <td class="black"><?
                                foreach($lf['_sv'] as $k=>$v){
                                    ?><label<?=$v['chk']?' class="active"':''?>><input type="checkbox"<?=$v['chk']?' checked':''?> name="_sv[<?=$k?>]" id="<?=$v['id']?>"  value="1"><i></i><span class="sp-label-wrapper"><button type="submit" class="mobile-result-label">Найти <span class="mobile-result-label__value"> <b>-</b> шт.</span></button><?=$v['anc']?></span></label><?
                                }
                            ?></td><?
                        ?></tr><?
                    }
                    ...
                    <tr class="last d-f-control">
                        <td></td>
                        <td colspan="2"><input type="button" class="lfGo" value="найти"><div style="float:left;" class="loader"></div> <div class="result-label ext"></div></td>
                    </tr>
                </table>
            </form>
        </div>
