...
if(!empty($cat)){
    ...
    {
        echo $this->incView('catalog/disks/searchLentaTable');
    }
    else{
        global $app;
        echo $app->incView('catalog/disks/searchLentaTable');
    }
    ?>
    ...
    <? if(!empty($limit) && !empty($paginator)){
        ?><div class="showmore" id="showmore" onclick="showmore()" style="padding-bottom: 15px;"><div id="showmore_ajaxloading"></div><a>Показать еще</a></div><?
        ?><script>
            var cur_limit = <?=$limit?>;
            var lockPage = false;
        </script><?
