    ...
    case 'add':
        ...
        switch ($parent_name)
        {
            ...
            case 'year':
                $ab->query("SELECT * FROM ab_avto WHERE (avto_id='$parent_id')");
                $ab->next();
                $r->vendorId = $vendor_id = $ab->qrow['vendor_id'];
                $r->modelId = $model_id = $ab->qrow['model_id'];
                ...
                break;
            case '':
                $ab->query("INSERT INTO ab_avto (name,sname,manual_insert) VALUES('$v','$sname',1)");
                break;
            case 'vendor':
                $ab->query("INSERT INTO ab_avto (name,sname,{$parent_name}_id,manual_insert) VALUES('$v','$sname','$parent_id',1)");
                break;
        }
...
html;
    ...
    case 'tyres':
        ...
        $i = 0;
        foreach ([10, 15, 18, 12] as $type_id)
        {
            if (isset($ab->avto[1][$type_id])) foreach ($ab->avto[1][$type_id] as $k => $v)
            {
                ...
                $r->rows[$i]['cell'] = [
                    ...
                    $v['pos'],
                    '',
                    array_search($type_id, [10, 12]) !== false ? 'заводская' : 'альтернатива',
                    $v['P3'],
                    $v['P2'],
                    $v['P1'],
                ];
                if (isset($ab->avto[1][$type_id + 1][$k])) $r->rows[$i]['cell'] = array_merge($r->rows[$i]['cell'], [
                    'задняя ось',
                    $ab->avto[1][$type_id + 1][$k]['P3'],
                    $ab->avto[1][$type_id + 1][$k]['P2'],
                    $ab->avto[1][$type_id + 1][$k]['P1'],
                    $v['manual_insert'],
                ]);
                else $r->rows[$i]['cell'] = array_merge($r->rows[$i]['cell'], ['задняя ось', '', '', '', $v['manual_insert']]);
                $i++;
            }
        }
    ...
    case 'disks':
        ...
        $ab->avto_sh0($modif_id);
        $i = 0;
        foreach ([22, 28, 20, 25] as $type_id)
        {
            if (isset($ab->avto[2][$type_id])) foreach ($ab->avto[2][$type_id] as $k => $v)
            {
                $r->rows[$i]['id'] = $v['avtosh_id'];
                $r->rows[$i]['cell'] = [
                    $v['avtosh_id'],
                    $v['pos'],
                    '',
                    array_search($type_id, [20, 22]) !== false ? 'заводская' : 'альтернатива',
                    $v['P2'],
                    $v['P5'],
                    $v['P1'],
                    $v['P4'],
                ...
                if (isset($ab->avto[2][$type_id + 1][$k])) $r->rows[$i]['cell'] = array_merge($r->rows[$i]['cell'], [
                    'задняя ось',
                    $ab->avto[2][$type_id + 1][$k]['P2'],
                    $ab->avto[2][$type_id + 1][$k]['P5'],
                    $ab->avto[2][$type_id + 1][$k]['P1'],
                    $ab->avto[2][$type_id + 1][$k]['P4'],
                    $ab->avto[2][$type_id + 1][$k]['P6'],
                    $ab->avto[2][$type_id + 1][$k]['P3'],
                    $v['manual_insert'],
                ]);
                else $r->rows[$i]['cell'] = array_merge($r->rows[$i]['cell'], ['задняя ось', '', '', '', '', '', '', $v['manual_insert']]);
                $i++;
    ...
    case 'tyres_edit':
        ...
        else
        {
            $zad = false;
            $_type_id = 0;
            if ($p1_1 != 0 || $p2_1 != 0 || $p3_1 != 0) if ($p1_1 == 0 || $p2_1 == 0 || $p3_1 == 0)
            {
            ...
            else if ($type == 'zavod' && $zad)
            {
                $type_id = 12;
                $_type_id = 13;
            }
            else if ($type == 'zamena' && !$zad) $type_id = 15;
            else if ($type == 'zamena' && $zad)
            {
                $type_id = 18;
                $_type_id = 19;
            }
    ...
    case 'disks_edit':
        ...
        else
        {
            $zad = false;
            $_type_id = 0;
            if ($p2_1 != 0 || $p5_1 != 0 || $p1_1 != 0) if ($p1_1 == 0 || $p5_1 == 0 || $p2_1 == 0)
            {
                $r->fres_msg = 'Задняя ось не записана. Не все значения указаны';
                $r->fres = -1;
            }
            ...
            else if ($type == 'zamena' && !$zad) $type_id = 25;
            else if ($type == 'zamena' && $zad)
            {
                $type_id = 28;
                $_type_id = 29;
    ...
    case 'save_common':
        ...
        if (!empty($modif_id) && !empty($_REQUEST['cdata']))
        {
            ...
            {
                $dt = str_replace(['x', ','], ['*', '.'], strtolower($dt));
            }
