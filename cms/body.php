...
<div class="row">
    ...
    <div class="column">
        <fieldset>
            ...
            if (MC::chk())
            {
                ...
                if (empty($v)) warn('<b style="color: red">ВНИМАНИЕ!</b> Задание по обновлению базы данных (cron/cron15) не запускалось более 30 минут. Возможно это нарушение настроек CRON сервера. Необходимо вмешательство администратора сайта.');

                if (Data::get('SMS_enabled'))
                {
                    $sms = new Bot_SMS();
                    $v = MC::get($sms->mcBotName());
                    if (empty($v)) warn('<b style="color: red">ВНИМАНИЕ!</b> Давно не выполнялось задание по контролю доставки СМС сообщений (cron/smsChk). Возможно это нарушение настроек CRON сервера. Необходимо вмешательство администратора сайта.');
            ...
            if (0 && MDB::connect())
            {
                $r = MDB::$db->command(['buildinfo' => true]);
    ...
    <div class="row">
        ...
        <div class="column">
            <fieldset>
                <legend>Статистика базы данных</legend><?
    ...
    <div class="row">
        <div>
            <fieldset>
                <legend>Статистика звонков и заказов</legend><?
                ...
                $d = $os->fetchAll("SELECT DATE(dt_add) AS date1, COUNT(*) AS ordersNum FROM os_order oo WHERE NOT LD AND DATE(dt_add) >= DATE_SUB(DATE(NOW()), INTERVAL 1 DAY) GROUP BY date1 ORDER BY date1 DESC", MYSQLI_ASSOC);
                echo '<p><b>ЗАКАЗОВ сегодня/вчера: ';
                $orders = [0, 0];
                if (count($d) == 2) $orders = [$d[0]['ordersNum'], $d[1]['ordersNum']];
...
<? $ss = new Content();
$t = $ss->getDoc('cms_mp');
if (CMS_LEVEL_ACCESS == 1) $t1 = $ss->getDoc('cms_mp_level1');
else $t1 = '';
...
if (Cfg::get('botLog'))
{
    ...
    <div id="botLog" style="width:99%; display:none; overflow:visible; margin-top:15px;">
        <fieldset class="ui" style="border:1px dashed #039; padding:15px; margin:10px 0">
            <legend class="ui">Статистика посещений сайта поисковыми ботами</legend>
            <div id="botlogPager"></div>
            <table id="botlogGrid"></table>
            <table id="info" class="ui-table">
                <tr>
                    <th>За сегодня Yandex/Гугл</th>
                    <td id="today"><span></span> / <span></span></td>
                </tr>
                <tr>
                    <th>Вчера Yandex/Гугл</th>
                    <td id="yesteday"><span></span> / <span></span></td>
                </tr>
                <tr>
                    <th>За неделю Yandex/Гугл</th>
                    <td id="week"><span></span> / <span></span></td>
                </tr>
                <tr>
                    <th>За 30 дней Yandex/Гугл</th>
                    <td id="month"><span></span> / <span></span></td>
                </tr>
                <tr>
                    <th>Всего записанных в лог посещений Yandex/Гугл</th>
                    <td id="total"><span></span> / <span></span></td>
                </tr>
            </table>
        </fieldset>
    </div>
