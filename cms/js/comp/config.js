...
/* MIN_EXTRA   */
	...
	$('#me_grid1').jqGrid({
		...
		onSelectRow: function(id){ 
				if(me.lastsel1) jQuery('#me_grid1').jqGrid('restoreRow',me.lastsel1); 
				jQuery('#me_grid1').jqGrid('editRow',id,true,null,function(xhr){
					if(xhr.readyState==4){
						if(xhr.responseText==0) $('#me_grid1').trigger('reloadGrid'); else return true;
					}
				}); 
				me.lastsel1=id; 
		}
	...
	$('#me_grid2').jqGrid({
		...
		loadComplete: loadComplete,
		onSelectRow: function(id){ 
				if(me.lastsel2) jQuery('#me_grid2').jqGrid('restoreRow',me.lastsel2); 
				jQuery('#me_grid2').jqGrid('editRow',id,true,null,function(xhr){
					if(xhr.readyState==4){
						if(xhr.responseText==0) $('#me_grid2').trigger('reloadGrid'); else return true;
					}
				}); 
				me.lastsel2=id; 
		}
...
//		autowidth: true,
		...
		loadComplete: loadComplete,
		onSelectRow: function(id){ 
				if(os_lastsel) jQuery('#os_grid').jqGrid('restoreRow',os_lastsel); 
				jQuery('#os_grid').jqGrid('editRow',id,true,null,function(xhr){
					if(xhr.readyState==4){
						if(xhr.responseText==0) $('#os_grid').trigger('reloadGrid'); else return true;
					}
				}); 
				os_lastsel=id; 
		}
...
//		height:'100%',
		...
		loadComplete: loadComplete,
		onSelectRow: function(id){ 
				if(curval_lastsel) jQuery('#curval_grid1').jqGrid('restoreRow',curval_lastsel); 
				jQuery('#curval_grid1').jqGrid('editRow',id,true,null,function(xhr){
					if(xhr.readyState==4){
						if(xhr.responseText==0) $('#curval_grid1').trigger('reloadGrid'); else return true;
					}
				}); 
				curval_lastsel=id; 
		}
	...
	$('#ringsWin').dialog({
		...
		open: function(){
			...
			$('#ringsGrid').jqGrid({
				...
				loadComplete: loadComplete,
				onSelectRow: function(id){ 
					if(rings_lastsel) jQuery('#ringsGrid').jqGrid('restoreRow',rings_lastsel); 
					jQuery('#ringsGrid').jqGrid('editRow',id,true,null,function(xhr){
						if(xhr.readyState==4){
							if(xhr.responseText==0) $('#ringsGrid').trigger('reloadGrid');  else return true;
						}
					}); 
					rings_lastsel=id; 
				}
