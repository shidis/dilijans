...
(function($) {
    ...
    $.fn.contextmenu = function(option) {
        option = $.extend({ alias: "cmroot", width: 150, leftclick:false , cursor:'pointer'}, option);
...
//					if(option.leftclick) $(tmp).bind("click", returnfalse).hover(overItem, outItem);
                } 
        ...
        var me = $(this).each(function() {
			...
			if(option.leftclick) $(this).bind('click', function(e) {
                var bShowContext = (option.onContextMenu && $.isFunction(option.onContextMenu)) ? option.onContextMenu.call(this, e) : true;
                if (bShowContext) {
                    if (option.onShow && $.isFunction(option.onShow)) {
                        option.onShow.call(this, root);
