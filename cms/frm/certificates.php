...
$gr = 2;
$cp->frm['name']='certificates';
$cp->frm['title']='Сертификаты дисков';
...
<script language="JavaScript">
    ...
    $('document').ready(function()
    {
        $('#at_frm').submit(function(e){
            e.preventDefault();
            doLoad('save_discr',{'cert_id': $('#at_frm input[name=cert_id]').val(),'gtext': $('#at_frm #gtext').val()});
            $('#g'+$('#at_frm input[name=cert_id]').val()+' a').attr('onClick','return hs.expand(this, {captionText:"'+$('#at_frm #gtext').val()+'"})');
            $('#at_frm #gtext').val('');
        });
...
//	  .jqResize('.jqResize')
            .jqm({
                ...
                onShow: function(h) {
                    ...
                    $('#at_frm input[name=cert_id]').val(h.t.value);
                    doLoad('load_discr',{cert_id: h.t.value});
                },
                onHide: function(h) {
    ...
    function doLoad(act, data) {
        ...
        switch (act) {
            ...
            case 'brands':
                ...
                req.onreadystatechange = function () {
                    if (req.readyState == 4) {
                        ...
                        //document.getElementById('debug').innerHTML = req.responseJS.debug;
                        document.getElementById('cert_loader').innerHTML = '';
                    }
                }
                break;
            case 'brand_sel':
                ...
                break;
            case 'model_sel':
                loading('cert_loader', 'large');
                req.onreadystatechange = function () {
                    if (req.readyState == 4) {
                        document.getElementById('cert_loader').innerHTML = req.responseText;
                        teditWinBind();
                        //document.getElementById('debug').innerHTML = req.responseJS.debug;
                    }
                }
            ...
            case 'first':
                ...
                break;
            case 'model_sel_upd':
                loading('cert_data', 'large');
                req.onreadystatechange = function () {
                    if (req.readyState == 4) {
                        document.getElementById('cert_data').innerHTML = req.responseText;
                        teditWinBind();
                        //document.getElementById('debug').innerHTML = req.responseJS.debug;
                    }
                }
            ...
            case 'options_save':
                ...
                break;
            case 'delete':
                ss = document.getElementById('g' + data.cert_id).innerHTML;
                saving('g' + data.cert_id, 'g' + data.cert_id, '<br><br>Удаляю...');
                req.onreadystatechange = function () {
                    if (req.readyState == 4) {
                        //document.getElementById('debug').innerHTML = req.responseJS.debug;
                        saved('g' + data.cert_id, 'g' + data.cert_id, 'Удалено');
                        if (!req.responseJS.deleted) {
                            document.getElementById('g' + data.cert_id).innerHTML = ss;
                        } else document.getElementById('g' + data.cert_id).innerHTML = req.responseJS.block;
                    }
                }
                break;
            case 'cert_add':
                saving('cert_add_sbut', 'cert_add_saving', '');
                req.onreadystatechange = function () {
                    if (req.readyState == 4) {
                        //document.getElementById('debug').innerHTML = req.responseJS.debug;
                        saved('cert_add_sbut', 'cert_add_saving', req.responseText);
                        if (req.responseJS.cert_id > 0) {
                            doLoad('model_sel_upd', {model_id: req.responseJS.model_id});
                            document.getElementById('cert').innerHTML += req.responseJS.block;
                            teditWinBind();
                        }
                    }
                }
        ...
        req.caching = false;
        req.open(null, '../be/certificates.php?gr=<?=$gr?>&act='+act, true);
...
<fieldset class="ui"><legend class="ui">Сертификаты</legend>
<form>
<div style="float:left;padding-right:30px;" id="brands"></div>
<div style="" id="models"></div>
</form>
<div style=" padding-top:10px" id="cert_loader"></div>
</fieldset>
<?
if(in_array(CMS_LEVEL_ACCESS,array(1,2))){
?>
<fieldset class="ui"><legend class="ui">Настройки сертификатов</legend>
